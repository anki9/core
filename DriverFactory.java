...
public class DriverFactory {
    ...
    /***
     * Creates and returns instance of Chrome driver emulating the display of a mobile device
     * Defaults to Nexus 5, can be changed with -DemulatedDevice=DeviceName
     * Valid device names can be found in Chrome > Dev Tools > Hamburger > Settings > Devices
     * Or Chrome > {F12} > {F1} > Devices
     *
     * @return Chrome driver
     */
    private static WebDriver createChromeMobileEmulationDriver() {
        Map<String, String> mobileEmulation = new HashMap<>();

        mobileEmulation.put("deviceName", Config.getEmulatedDevice());
    ...
    private static void addAllBrowserSetup(WebDriver driver) {
        ...
        if (!Config.isMobile() && Config.getWindowSize().equals("maximized"))
            driver.manage().window().maximize();
        if (!Config.getWindowSize().equals("maximized"))
            driver.manage().window().setSize(
                    new Dimension(Integer.parseInt(Config.getWindowSize().toLowerCase().split("x")[0]),
                            Integer.parseInt(Config.getWindowSize().toLowerCase().split("x")[1])));
    }
    ...
    private static String getDriverPath() {
        ...
        if (Config.getBrowser().equalsIgnoreCase(Constants.SAFARI_BROWSER)) {
            ...
            driverPath = driverPath + "IEDriverServer.exe";
                ...
                Config.getBrowser().equalsIgnoreCase(Constants.CHROME_MOBILE_EMULATION_BROWSER)) {
    ...
    private static String getRemoteUrl() {
        ...
        } else if (Config.getBrowser().equalsIgnoreCase(Constants.CHROME_BROWSER) || Config.getBrowser().equalsIgnoreCase(Constants.CHROME_MOBILE_EMULATION_BROWSER)) {
